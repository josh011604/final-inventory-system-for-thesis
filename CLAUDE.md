...
- **Auto-approved borrows land as `status = 'borrowed'`, not `'confirmed'`** — there is no approval step to wait for and no separate hand-over, so the item is in the borrower's hands immediately. `'confirmed'` means "approved by someone else, "; both are active everywhere it matters (`ACTIVE_BORROW_STATUSES`, the stock trigger, the overdue sweep, Mark Returned, the dashboard/report "still out" filters).
- **Auto-approved borrows are inserted with the service-role client**, because the `borrow insert scoped` policy pins every client insert to `status = 'pending'` — a caller's own JWT cannot create an already-approved row. They must therefore stamp `approved_by`/`approved_by_name`/`approved_by_role`/`approved_at` themselves, since `transition_borrow_record` (which normally does it) is bypassed — miss that and the approval reads as "—". That means `auth.uid()` is NULL inside `audit_row_change()`, so it falls back to the row's `created_by` / `requester_id` / `borrower_id` (migration `20260729170000`) to keep the entry attributed to the admin rather than to "System". `approved_by` is set to the borrower themselves on this path — that is what the UI's "· auto" marker keys off, not a bug.
...
- **Notifications are personal, and `profile_id IS NULL` means "an approver in this department must act"** (migration `20260729230000`). The select/update policies are `profile_id = auth.uid()` plus, for a department admin only, that department's unaddressed approval alerts — so a super admin no longer sees every notification in the system and a faculty member no longer reads their admin's queue. Central/Supply Office approvals are inserted per super admin *with* `profile_id`, which is why they still arrive. Clients cannot insert notifications at all (there is no insert policy); everything comes from a `security definer` trigger or `notify_password_changed()`.
- **Approval routing falls out of `borrow_records.department_id` (= the item's department) plus read RLS**, not from any explicit routing table: a Supply Office request carries `department_id = NULL`, which no department admin's department can equal, so it reaches only the super admin — and the `borrow select scoped` policy keeps it out of their list entirely. Faculty approval of a student's request additionally needs the borrower's *role*, which the `profiles` policy hides from faculty. **Identity on a borrow record is denormalized, never joined**: `borrower_name`/`borrower_role` are stamped by the `enforce_borrow_department_scope` trigger and `approved_by_name`/`approved_at`/`returned_by_name` by `transition_borrow_record` (migrations `20260729120000`, `130000`, `180000`) — because `profiles` RLS silently nulls those joins for every non-admin viewer. Don't "fix" the missing joins by re-adding them.
- **Inventory Items is browsable across departments, one tab per department** (`ALL_TAB` / department id / `SUPPLY_TAB` in `InventoryPage.tsx`), so an item owned by another department is found and borrowed straight from its tab; Borrowing → New Request is the second route to the same thing and groups its picker by department the same way. **Both derive their groups from the rows the query returned, never from a role check** — equipment RLS already applies the rule (a student reads every departmental item but no Supply Office stock; faculty/department admins/super admin read both), so a Supply Office tab or group simply never materialises for a student. Don't re-derive that rule in the view. Editing is gated separately per row by `canEditItem()`, because `equipment admin write` still scopes a department admin to their own department — browsing another department must not offer an Edit the database will reject. The New Request picker (`useBorrowCandidates`) offers **every** department's stock to every role — scoping it by the borrower's own department made two approval rules unreachable (a super admin has no department, and a department admin could never reach another department's items). Scope restrictions that remain: students cannot request Supply Office (department-less) items — enforced client-side by `borrowScopeReason()`, in the `borrow-status` create path, and by the `enforce_borrow_department_scope` trigger (migration `20260729180000`).
