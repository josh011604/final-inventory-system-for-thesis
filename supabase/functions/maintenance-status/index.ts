...
const VALID_STATUSES = ['approved', 'rejected', 'in_progress', 'completed']
...
Deno.serve(async (req) => {
	...
	if (typeof id !== 'number' || typeof status !== 'string' || !VALID_STATUSES.includes(status)) {
		return json({ error: 'Body must be { id: number, status: approved|rejected|in_progress|completed }' }, 400)
	}
	...
	// only ever acts through transition_maintenance_request(), which re-checks
	// the actor's role/department before writing anything.
	const adminClient = createClient(supabaseUrl, serviceRoleKey)

	const { data, error } = await adminClient
		.rpc('transition_maintenance_request', {
			p_request_id: id,
			p_new_status: status,
